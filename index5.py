...
def query_approval_prompt(context: RunContextWrapper[ResearchContext], agent: Agent[ResearchContext]) -> str:
    ...
    **EXAMPLES:**
    Query: "Explain attention mechanisms in AI?" → handoff to Planning Agent
    Query: "Research AI" → handoff to Requirement Gathering Agent  
    Query: "Tell me about quantum physics" → {{"status": "rejected", "reason": "Quantum physics not in user interests"}}
    Query: "What is my name?" → {{"status": "rejected", "reason": "Meta-question about user profile"}}
    """
...
async def test_rejection_handling():
    ...
    for query in test_queries:
        ...
        if is_rejection_response(output.final_output):
            ...
            print(f"❌ Should have been rejected but wasn't: {output.final_output}")
