...
def planning_agent_prompt(context: RunContextWrapper[ResearchContext], agent: Agent[ResearchContext]) -> str:
    ...
    User: {user_name}
    Specialized Research Interests: {intercepts_list}
        ...
        while True:
            ...
            if approved:
                ...
                if hasattr(user_context, 'cited_results') and user_context.cited_results:
                    print(f"\n📊 Research included {len(user_context.cited_results.get('sources', []))} properly cited sources")
            else:
                print(f"\n❌ {result}")
...
# Test function to see the enhanced research process
async def test_enhanced_research():
    """Test function to demonstrate the enhanced research process with citation and reflection"""
    print("Testing enhanced research process with citation and reflection...")
    
    # Create test context
    test_context = ResearchContext(
        profile=UserProfile(name="TestUser", interests=["AI", "Technology"], results_count=2)
    )
    ...
    for query in test_queries:
        ...
        if approved:
            print("✅ Research completed with citation and reflection!")
            print(f"📝 Final report length: {len(result)} characters")
            
            # Check if we have cited results
            if hasattr(test_context, 'cited_results') and test_context.cited_results:
                print("📚 Citations were properly added")
        else:
...
if __name__ == "__main__":
    # Uncomment to test enhanced research process
