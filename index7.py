"""
This is my deep research agent project with proper handoff handling
"""
...
def query_approval_prompt(context: RunContextWrapper[ResearchContext], agent: Agent[ResearchContext]) -> str:
    ...
    **EXAMPLES:**
    Query: "What is nginx?" → handoff to Planning Agent (related to DevOps)
    Query: "Research AI" → handoff to Requirement Gathering Agent (needs clarification)
    Query: "How to make pasta?" → {{"status": "rejected", "reason": "Cooking not in user interests"}}
    """
...
async def run_agent_chain(starting_agent: Agent, input_text: str, context: ResearchContext) -> Tuple[bool, str]:
    ...
    for iteration in range(max_iterations):
        ...
        if (current_agent == requirement_gathering_agent and 
            hasattr(context, 'requirement_questions') and 
            context.requirement_questions):
        ...
        # For other agents, check if they provided output that needs processing
        # For now, we'll assume they hand off correctly using the handoff mechanism
        # and we'll just move to the next agent in our predefined chain
        
        # Simple agent chain logic
        if current_agent == approval_router:
            # Approval router should hand off to either requirement or planning
            # For simplicity, we'll check the query type
            if "research" in current_input.lower() or "study" in current_input.lower():
...
# Test function to demonstrate the improved system
async def test_system():
    """Test function to demonstrate the system"""
    print("Testing the research agent system...")
    ...
    # Test queries
    test_queries = [
        "What is nginx?",  # Should be approved
        "Research AI",  # Should go to requirement gathering
        "How to make pasta?",  # Should be rejected
    ]
